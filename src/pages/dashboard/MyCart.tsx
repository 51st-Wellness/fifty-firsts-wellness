...
const MyCart: React.FC = () => {
  const { items, totalPrice, updateCartItem, removeFromCart, isLoading } = useCart();
    ...
    return (
      <div className="rounded-lg">
        ...
        <div className="text-center py-16 bg-gray-50 rounded-xl border border-gray-200">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1" style={{ fontFamily: '"League Spartan", sans-serif' }}>Your cart is empty</h3>
          <p className="text-gray-500 text-sm mb-6">Browse the marketplace to add products.</p>
          <Link to="/marketplace" className="inline-block px-6 py-3 rounded-full bg-brand-green text-white font-semibold hover:bg-brand-green-dark">
            Go to Marketplace
          </Link>
        </div>
      </div>
    ...
    <div className="rounded-lg">
      ...
      <div className="space-y-4">
        {items.map((ci) => {
          ...
          return (
            <div key={ci.id} className="bg-white rounded-xl p-4 flex items-start gap-4 relative">
              {/* Image left */}
              <img
              ...
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-gray-900 line-clamp-1">{storeItem?.name}</div>
                <div className="text-sm text-gray-500 mt-1">Category: {category}</div>
                
                {/* Bottom right of image: quantity controls */}
                <div className="flex items-center gap-2 mt-3">
                  <button
                    onClick={() => updateCartItem(ci.productId, Math.max(1, ci.quantity - 1))}
                    disabled={isLoading}
                  ...
                  <button
                    onClick={() => updateCartItem(ci.productId, ci.quantity + 1)}
                    disabled={isLoading}
                    className="w-8 h-8 rounded-full bg-brand-green hover:bg-brand-green-dark flex items-center justify-center disabled:opacity-50"
                  >
              ...
              <div className="absolute top-4 right-4">
                <div className="text-lg font-semibold text-gray-900">{formatPrice(storeItem?.price ?? 0)}</div>
              </div>
        ...
        <div className="mt-8 bg-white rounded-xl p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center pb-3 border-b border-gray-200">
              <span className="text-gray-600">Item's total ({items.length}):</span>
            ...
            <p className="text-sm text-gray-500 pb-3 border-b border-gray-200">Delivery fees not included yet.</p>
            <button 
