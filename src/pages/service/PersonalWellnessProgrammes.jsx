...
const PersonalWellnessProgrammes = () => {
  return (
    <main className='px-4 sm:px-6 lg:px-12'>
      {/* Header Section */}
      <article className='flex flex-col-reverse md:flex-row justify-between items-center md:items-start w-full gap-4'>
        {/* Left Content */}
        <div className='flex flex-col gap-4 w-full md:w-1/2 text-center md:text-left'>
          ...
          <div className='text-sm sm:text-base text-[#475464]'>
            Choose a program designed to support your unique wellness goals — from reducing stress to building mindful routines, one step at a time.
          </div>
        </div>
      ...
      <article className='flex flex-col sm:flex-row justify-between items-center mt-6 gap-4'>
        ...
        <section className='flex flex-wrap justify-center sm:justify-start gap-3 items-center'>
          <div className='text-[#4444B3] border-[#4444B3] rounded-full border py-1 px-3 text-sm font-medium'>All</div>
          <div className='text-sm font-medium cursor-pointer'>Stress Relief</div>
          <div className='text-sm font-medium cursor-pointer'>Better Sleep</div>
          <div className='text-sm font-medium cursor-pointer'>Mindfulness</div>
      ...
      <article className="flex flex-col p-4 sm:p-6 w-full mt-6 items-center">
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          {[meditate, stare, sleep, meditate, stare, sleep, meditate, stare, sleep].map((img, idx) => (
            <div key={idx} className="flex flex-col gap-2 border rounded-3xl p-4 hover:shadow-md transition">
